...
async function carregarInit() {
  try {
    ...
    dataFav.forEach(fav => {
      ...
      newItem.innerHTML = `
        <img src="${fav.candidatos.foto}" alt="Logo ${fav.candidatos.nome}" class="favorito-logo w-10 h-10 rounded-full object-cover border border-gray-600 bg-white shadow-sm">
        <div class="favorito-info">
        <div class="favorito-nome">${fav.candidatos.nome}</div>
      ...
      const html = `
        <article
          ...
          <div
            ...
            <img src="${cand.foto}" alt="Logo Tech Solutions"
            ...
            <button
              id="${cand.id}"
              class="favorite-btn absolute top-3 right-3 bg-black/40 border-none w-9 h-9 rounded-full flex items-center justify-center cursor-pointer text-gray-300 transition-all z-20 backdrop-blur-sm hover:bg-black/60 hover:scale-110 ${candidatosJaFavoritados.includes(cand.id)?' active':""}"
              aria-label="Marcar como favorito">
              <i class='bx bx-star text-xl'></i>
            </button>
          </div>
          <div class="card-body p-5 flex-1">
            <h3 class="empresa-nome text-xl font-semibold mb-2 text-white">${cand.nome}</h3>
            <p class="empresa-descricao text-gray-400 text-sm mb-4 line-clamp-3 leading-relaxed">
              ${cand.descricao?cand.descricao:''}
            </p>
          </div>
          <div
            ...
            <div class="card-meta flex items-center gap-2 flex-1 min-w-[150px]">
              ...
              ${
                cand.tags[0]?
                `<span
                  class="tag-chip text-white px-2.5 py-1 rounded-full text-xs cursor-pointer transition-all">${cand.tags[0].nome}</span>
                `
                :''
              }
              ${
                cand.tags[1]?
                `<span
                  class="tag-chip text-white px-2.5 py-1 rounded-full text-xs cursor-pointer transition-all">${cand.tags[1].nome}</span>
                `
                :''
              }
              ${
                cand.tags[2]?
                `<span
                  class="tag-chip text-white px-2.5 py-1 rounded-full text-xs cursor-pointer transition-all">${cand.tags[2].nome}</span>
                `
                :''
              }
              </div>
...
document.querySelector("#ver-pagina-btn").addEventListener("click", async() =>{
  ...
  try {
    ...
    const response = await axiosWe(`/candidatos/public?page=${pagina}`);
...
function setupFavoriteButtons() {
  ...
  favoriteButtons.forEach(btn => {
    btn.addEventListener('click', async function (e) {
      ...
      const candidatoId = this.id;

      // Verifica limite antes de favoritar
      if (!this.classList.contains('active') && candidatosJaFavoritados.length >= 30) {
        alert("Você só pode favoritar no maximo 30 candidatos!");
        return;
      }
      ...
      if (this.classList.contains('active')) {
        ...
        // Adiciona ao array
        candidatosJaFavoritados.push(candidatoId);

        // Adiciona a empresa aos favoritos na sidebar
        await addToFavorites(this.closest('.empresa-card'), candidatoId);
      } else {
        ...
        //Remove do Array 
        candidatosJaFavoritados = candidatosJaFavoritados.filter(cand => cand != candidatoId)

        // Remove a empresa dos favoritos na sidebar
        await removeFromFavorites(this.closest('.empresa-card'), candidatoId);
        
      }

      console.log(`Candidato ${this.classList.contains('active') ? 'adicionada aos' : 'removida dos'} favoritos: ${this.closest('.empresa-card').querySelector('.empresa-nome').textContent}`);
    });
  });
}
...
async function addToFavorites(card, cd) {
  ...
  try {
    await axiosWe.post('/favoritos/candidato', {cd});
  } catch (erro) {
...
async function removeFromFavorites(card, cd) {
  const empresaNome = card.querySelector('.empresa-nome').textContent;
  const favoritosContainer = document.getElementById('favoritos-container');
  const existingItems = favoritosContainer.querySelectorAll('.favorito-item');
  ...
  try {
    await axiosWe.delete(`/favoritos/candidato/${cd}`);
  } catch (erro) {
